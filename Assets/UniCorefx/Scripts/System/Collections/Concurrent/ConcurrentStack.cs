...
namespace System.Collections.Concurrent
{
    ...
    public class ConcurrentStack<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
    {
        ...
        object ICollection.SyncRoot
        {
            get
            {
                throw new NotSupportedException(_Environment.GetResourceString("ConcurrentCollection_SyncRoot_NotSupported"));
            }
        }
        ...
        private void ValidatePushPopRangeInput(T[] items, int startIndex, int count)
        {
            ...
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException("count", _Environment.GetResourceString("ConcurrentStack_PushPopRange_CountOutOfRange"));
            }
            int length = items.Length;
            if (startIndex >= length || startIndex < 0)
            {
                throw new ArgumentOutOfRangeException("startIndex", _Environment.GetResourceString("ConcurrentStack_PushPopRange_StartOutOfRange"));
            }
            if (length - count < startIndex) //instead of (startIndex + count > items.Length) to prevent overflow
            {
                throw new ArgumentException(_Environment.GetResourceString("ConcurrentStack_PushPopRange_InvalidCount"));
            }
        }
