...
namespace System.Threading
{
    ...
    public class ThreadLocal<T> : IDisposable
    {
        ...
        private T GetValueSlow()
        {
            ...
            if (id < 0)
            {
                throw new ObjectDisposedException(_Environment.GetResourceString("ThreadLocal_Disposed"));
            }
            ...
            else
            {
                ...
                if (IsValueCreated)
                {
                    throw new InvalidOperationException(_Environment.GetResourceString("ThreadLocal_Value_RecursiveCallsToValue"));
                }
            }
        ...
        public IList<T> Values
        {
            get
            {
                if (!m_trackAllValues)
                {
                    throw new InvalidOperationException(_Environment.GetResourceString("ThreadLocal_ValuesNotAvailable"));
                }

                var list = GetValuesAsList(); // returns null if disposed
                if (list == null) throw new ObjectDisposedException(_Environment.GetResourceString("ThreadLocal_Disposed"));
                return list;
            }
        }
        ...
        {
            if ((uint)minSize > _Array.MaxArrayLength)
            {
            ...
            if ((uint)newSize > _Array.MaxArrayLength)
            {
                newSize = _Array.MaxArrayLength;
            }
