    ...
    public class Lazy<T>
    {
        ...
        private static object GetObjectFromMode(LazyThreadSafetyMode mode)
        {
            if (mode == LazyThreadSafetyMode.ExecutionAndPublication)
                ...
                throw new ArgumentOutOfRangeException("mode", _Environment.GetResourceString("Lazy_ctor_ModeInvalid"));
        ...
        {
            return IsValueCreated ? Value.ToString() : _Environment.GetResourceString("Lazy_ToString_ValueNotCreated");
        }
        ...
        private Boxed CreateValue()
        {
            ...
            if (m_valueFactory != null)
            {
                try
                {
                    ...
                    if (mode != LazyThreadSafetyMode.PublicationOnly && m_valueFactory == ALREADY_INVOKED_SENTINEL)
                        throw new InvalidOperationException(_Environment.GetResourceString("Lazy_Value_RecursiveCallsToValue"));
            ...
            else
            {
                ...
                {
                    Exception ex = new global::System.MissingMemberException(_Environment.GetResourceString("Lazy_CreateValue_NoParameterlessCtorForT"));
                    if (mode != LazyThreadSafetyMode.PublicationOnly) // don't cache the exception for PublicationOnly mode
                        m_boxed = new LazyInternalExceptionHolder(ex);
                    throw ex;
                }
